...
int main(int argc, char *argv[])
{
    ...
    unsigned short seqNum = (rand() * rand()) % MAX_SEQN; //server side sequence number
    ...
    for (int i = 1;; i++)
    {
        ...
        while (1)
        {
            ...
            sendto(sockfd, &synackpkt, PKT_SIZE, 0, (struct sockaddr *)&cliaddr, cliaddrlen); //sending synack packet

            //polling for synack response, acking the synack
            while (1)
            {
                ...
                { //a packet has been recieved
                    ...
                    //if both statements fail, then the sequence numbers and ack numbers are incorrect
                }
            }
